...
class PendingOperations {
    ...
    lazy var downloadQueue: OperationQueue = {
        ...
        queue.name = "com.fikrihelmi.imagedownload"
...
class ViewController: UIViewController{
    ...
    lazy var tableView: UITableView = {
        ...
        v.register(UITableView.self, forCellReuseIdentifier: "movie-cell")
        return v
    }()
    
    private let _pendigOperations = PendingOperations()
    ...
    fileprivate func startDownload(movie: Movie, indexPath: IndexPath) {
        guard _pendigOperations.downloadInProgress[indexPath] == nil else {return}
        ...
        downloader.completionBlock = {
            ...
            DispatchQueue.main.async {
                self._pendigOperations.downloadInProgress.removeValue(forKey: indexPath)
                self.tableView.reloadRows(at: [indexPath], with: .automatic)
            }
        }
        
        _pendigOperations.downloadInProgress[indexPath] = downloader
        _pendigOperations.downloadQueue.addOperation(downloader)
    }
    
    fileprivate func toggleSuspendOperations(isSuspended: Bool){
        _pendigOperations.downloadQueue.isSuspended = isSuspended
    }
}
